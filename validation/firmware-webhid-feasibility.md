...
Chrome 官方 WebHID 规则保护 Mouse 等顶层 collection 中的报告。当前配置 Feature 位于 Mouse collection 内。桥停止后，Chrome 对已授权的两个同型号 HID 对象分别尝试 `open()`，两者均失败；另一浏览器环境可 `open()`，但 `sendFeatureReport(0, ...)` 被拒绝。这与受保护 collection 规则相符。RazerKit 的网页源码同样通过 WebHID 枚举已授权对象并逐个尝试，其 README 明确提示此类鼠标 collection 可能被浏览器拦截；用户在该网页也复现连接失败。
